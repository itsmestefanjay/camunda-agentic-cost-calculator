...
This repository contains browser-only calculators for Camunda agentic orchestration scenarios.

Primary artifacts:
- `index.html` — LLM cost comparison across models and volumes
- `project.html` — Project business case (ROI/NPV for an automation initiative)
- `style.css` — Shared design tokens and reusable components
...
- browser-native (no server, no build step)
- dependency-light
...
- Do not split logic across more files than necessary — keep each page self-contained with its own inline `<script>` and a `<style>` for page-specific CSS.
...
### Shared (`style.css`)
- Design tokens and theme variables (dark/light)
- Reusable components: `.panel`, `.settings-panel`, `.slider-group`, `.action-btn`, `.icon-btn`, `.executive-insight`, `.summary-card`, `.decision-badge`, `.chart-container`, `.chart-tabs`, `.chart-helper`, `.nav-drawer`, `.nav-backdrop`
- Any component used by more than one page belongs in `style.css`

### Cost Comparison (`index.html`)
- External dependency: Chart.js 4.x from CDN
- State persistence: `localStorage` (key: `camunda-cost-calc`) + `btoa`/`atob` URL hash sharing
- Scenario storage: separate `localStorage` key (`camunda-scenarios`)
- `recalculate()` is the single source of truth for: chart, table, breakeven info, summary cards, state persistence
- Prices stored internally as slider positions (not raw currency values); currency switching moves slider positions via `convertPrices(factor)`
- Five custom `afterDraw` plugins draw reference lines; a custom `Chart.Interaction.modes.intersectionOnly` prevents hover rendering bugs

### Project Business Case (`project.html`)
- External dependency: Chart.js 4.x from CDN
- State persistence: `localStorage` (key: `camunda-project-calc`)
- `recalculate()` pattern via `compute()` → update cards → update decision badge → render chart → update insights
- All monetary state stored in USD; currency display uses `displayRate()` multiplier (no slider position conversion)
- Two chart tabs: Cumulative ROI (line) and Annual Savings (bar)

### Navigation
- Burger menu button in every page header opens a `<nav class="nav-drawer">` with links to all pages
- The active page's nav item has class `active`; backdrop click and Escape close the drawer
...
- `style.css` — shared design tokens and UI components only. No page-specific layout or logic.
- `index.html` — page-specific layout, model-panel CSS, table CSS, all JS for cost comparison.
- `project.html` — page-specific layout, results CSS, all JS for project business case.
- Keep code sections clearly organized within each file: HTML structure, CSS (`<style>`), JavaScript (`<script>`).
- Use section markers (`/* ===== Section ===== */`) and consistent ordering so future edits remain tractable.
- A component belongs in `style.css` only if it is used on more than one page. Page-unique components stay in that page's `<style>` block.
